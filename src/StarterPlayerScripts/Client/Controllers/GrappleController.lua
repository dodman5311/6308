...
local function createRope(c, distance, a1)
	...
	newRope.WinchResponsiveness = 50
	newRope.WinchSpeed = 45
	newRope.WinchForce = 30000
	newRope.WinchTarget = 0
...
local function getObjectInCenter(blacklist, whitelist, ignoreDistance, maxDiff)
	local inCenter
	...
	for _, model in ipairs(CollectionService:GetTagged(whitelist or "Pickup")) do
		...
		if distanceUnit >= (maxDiff or 0.15) then
			continue
		end
...
local function detectHit(partHit: Part, launchedPart, item)
	...
	if
		partHit.CollisionGroup == "PathfindingHitbox"
...
function module.AltActivate(endPoint)
	...
	newAlign.Attachment0 = module.c.PrimaryPart:FindFirstChildOfClass("Attachment")
	newAlign.Parent = module.c

	local d = runService.RenderStepped:Connect(aimArm, newAlign)
	...
	acts:createTempAct("ability_invasive", function()
		...
		until distance <= 15
			...
			or (os.clock() - startTime >= 0.5 and not grapplePart.Weld.Part0)
...
local function grappleMovement()
	...
	local point = getObjectInCenter(nil, "GrapplePoint", true, 0.075)
	if point then
